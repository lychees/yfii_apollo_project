...
import {
  LIQUIDITY_FETCH_EXIT_BEGIN,
  LIQUIDITY_FETCH_EXIT_SUCCESS,
  LIQUIDITY_FETCH_EXIT_FAILURE,
} from './constants';
import { enqueueSnackbar } from '../../common/redux/actions'

export function fetchExit(index) {
  console.log('fetching exit...')
  return (dispatch, getState) => {
    // optionally you can have getState as the second argument
    dispatch({
      type: LIQUIDITY_FETCH_EXIT_BEGIN,
      index
    });
    ...
    const promise = new Promise(async (resolve, reject) => {
      ...
      contract.methods.exit().send({ from: address }).on(
        ...
        .on('receipt', function(receipt){
          ...
          dispatch({ type: LIQUIDITY_FETCH_EXIT_SUCCESS, index });
          resolve();
        })
        .on('error', function(error) {
          ...
          dispatch({ type: LIQUIDITY_FETCH_EXIT_FAILURE, index });
          resolve();
        })
        .catch((error) => {
          dispatch({ type: LIQUIDITY_FETCH_EXIT_FAILURE, index });
...
export function reducer(state, action) {
  ...
  switch (action.type) {
    case LIQUIDITY_FETCH_EXIT_BEGIN:
    ...
    case LIQUIDITY_FETCH_EXIT_SUCCESS:
    ...
    case LIQUIDITY_FETCH_EXIT_FAILURE:
